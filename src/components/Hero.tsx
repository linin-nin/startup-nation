      ...
      <div className="relative z-999">
        {/* social media logo  */}
            ...
            >
              {Socail.map((item) => (
                <Link href={item.path} key={item.logo}>
                  <Image src={item.logo} alt="icon" width={40} height={40} className="mt-5"/>
                </Link>
              ))}
            </div>
          </div>
          ...
          <div className=" h-full flex flex-grow justify-between">
            <h1
            className="text-[48px] font-bold glowy-text md:text-[6rem] md:tracking-[.425rem]">A Centaur of <br/> 
              <span className="text-[#F8DC4C]">Asia</span>
            </h1>
            <div>
              <p className=" w-[500px]">Lorem ipsum dolor sit amet consectetur. Risus vivamus ultrices pharetra consectetur vitae diam. Faucibus at suscipit justo non mauris in orci ut.</p>
              <Link href='/directory' className="w-full flex justify-end mt-5">
                <div className="border-2 px-5 py-2 border-[#FAFF00] flex justify-end items-center">
                  <p>Explore <span className="text-[#FAFF00]">Startup</span></p>
